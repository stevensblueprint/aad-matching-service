...
def process_form_data(form_data_path):
    ...
    processed_data_df = raw_data_df[["ID", "Mentor or Mentee?", "Rank 1", "Rank 2", "Rank 3", "Rank 4"]]

    # Step 2: Process Mentor and Mentee-specific columns
    mentor_columns = {
        "preferences": list(raw_data_df.columns[20:35]),  # Preferences for mentors (columns 20-34)
        "industry": raw_data_df.columns[16],              # Industry column for mentors
        "rankings": list(raw_data_df.columns[16:18]),     # Additional ranking questions
    }

    mentee_columns = {
        "preferences": list(raw_data_df.columns[39:54]),  # Preferences for mentees (columns 39-53)
        "industry": raw_data_df.columns[35],              # Industry column for mentees
        "rankings": list(raw_data_df.columns[35:37]),     # Additional ranking questions  
    }
    ...
    # Process Mentor Data
    mentors_processed = mentors[["ID"] + mentor_columns["preferences"] + ["Rank 1", "Rank 2", "Rank 3", "Rank 4"]]
    mentors_processed = mentors_processed.rename(columns={col: f"Preference {i+1}" for i, col in enumerate(mentor_columns["preferences"])})
    mentors_processed["Industry"] = mentors[mentor_columns["industry"]]

    # Process Mentee Data
    mentees_processed = mentees[["ID"] + mentee_columns["preferences"] + ["Rank 1", "Rank 2", "Rank 3", "Rank 4"]]
    mentees_processed = mentees_processed.rename(columns={col: f"Preference {i+1}" for i, col in enumerate(mentee_columns["preferences"])})
    mentees_processed["Industry"] = mentees[mentee_columns["industry"]]

    # Combine the processed data
    processed_data_df = pd.concat([mentors_processed, mentees_processed], ignore_index=True)
